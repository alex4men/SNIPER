...
def convert_tsv2coco(tsvPaths, outPath):

    id_images = 1

    if not os.path.exists('coco_format'):
        os.mkdir('coco_format')
    ...
    for tsvPath in tsvPaths:
        ...
        for rawImgPath in tqdm(imgPathList):
        ...
        print('Annotated', id_images-1, 'images, from the total', len(tsv_data),'images in the index.tsv)')
...
if __name__ == '__main__':
    ...
    parser.add_argument('tsvfiles', metavar='index.tsv', nargs='+', help='an integer for the accumulator')
    parser.add_argument('-o', '--out_json', nargs='?', default='/Users/alex/Documents/Programming/RoadAR/annotations/coco_format/instances_final.json', help='define the output filename, like instances_set_name.json')

    args = parser.parse_args()

    print('Converting final')
    convert_tsv2coco(args.tsvfiles, args.out_json)
